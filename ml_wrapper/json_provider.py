...
FORMAL_REPLACES = {
    ...
    for ref in ["analyses-formal.json", "data-formal.json", "ml-formal.json"]
}
...
def _combine_ml_and_example(file_, analyse=True):
    result = _read_json(
        "ml-analyses-example.json" if analyse else "ml-update-example.json"
    )
    with open(join(SCHEMA_DIR, file_)) as file:
...
ANALYSES_FORMAL = _read_json("analyses-formal.json")
...
JSON_ML_ANALYSE_TIME_SERIES = _combine_ml_and_example(
    "analyses-example-time_series.json"
)
JSON_ML_ANALYSE_MULTIPLE_TIME_SERIES = _combine_ml_and_example(
    "analyses-example-multiple_time_series.json"
)
JSON_ML_ANALYSE_TEXT = _combine_ml_and_example("analyses-example-text.json")
...
JSON_ANALYSE_TIME_SERIES = _read_json("analyses-example-time_series.json")
JSON_ANALYSE_MULTIPLE_TIME_SERIES = _read_json(
    "analyses-example-multiple_time_series.json"
)
JSON_ANALYSE_TEXT = _read_json("analyses-example-text.json")
